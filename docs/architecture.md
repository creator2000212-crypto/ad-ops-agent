...
项目将 WorkBuddy 已使用的业务工作流提炼为宿主指令，同时保留独立的 Python 验证路线。历史案例、宿主当前能力和 Python 实现是三类证据，不能相互替代。[能力范围](capability-status.zh-CN.md)列出各自边界，[脱敏案例](case-studies.zh-CN.md)说明来源。
...
自然语言交流、媒体查看及平台调用由宿主实际可用的能力承担。不能因为本地 Python 没有这些功能，就禁止宿主使用已有工具；也不能因为导入了说明文件，就宣称宿主已具备能力。部署者应按本次平台、账户、广告产品、工具版本与权限验证。历史 WorkBuddy 使用不等于另一宿主已经通过验收。
