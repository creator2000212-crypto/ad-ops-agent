...
项目将已在 Agent 工具中使用的业务工作流提炼为宿主指令，同时保留独立的 Python 验证路线。历史案例、宿主当前能力和 Python 实现是三类证据，不能相互替代。[能力范围](capability-status.zh-CN.md)列出各自边界，[脱敏案例](case-studies.zh-CN.md)说明来源。

架构分开三类职责：宿主模型理解任务并提出候选；检查步骤与已实现的确定性规则核对范围、字段、预算与依赖；获授权工具执行具体操作并提供可核验结果。模型说“已经准备好”不能替代检查，工具回包成功也不能替代对象读回。

## 宿主工作路线与离线验证路线

```mermaid
flowchart TD
    U["用户当前任务 与已有授权"] --> H["宿主模型和会话"]
    I["项目工作流 公共知识"] --> H
    B["本次相关私有资料 方法与记录"] --> H
    H --> P["当前对象读取 与可审阅方案"]
    P --> A["核对具体范围 能力与授权"]
    A --> T["宿主已有 MCP API SDK 或获准后台操作"]
    T --> E["原生读取结果 回执与未完成项"]
    E --> H
    H -. 可选结构化输入 .-> O["Python 离线检查 编排和模拟"]
    O -. 本地验证结果 .-> H
```

**宿主路线**从当前任务开始：复核所需账户和工具能力，复用相关业务资料，准备结果或整批差异，核对现有授权后执行，并按对象读回。只读报表不需要发布权限；素材审查、归属排查、迁移和清理也不必先转换成素材测试。步骤与交接字段见[宿主工作流](workflow.zh-CN.md)和[任务记录模板](../templates/task-record.zh-CN.md)。

自然语言交流、媒体查看及平台调用由宿主实际可用的能力承担。不能因为本地 Python 没有这些功能，就禁止宿主使用已有工具；也不能因为导入了说明文件，就宣称宿主已具备能力。部署者应按本次平台、账户、广告产品、工具版本与权限验证。某个 Agent 工具的历史使用不等于另一宿主已经通过验收。

**Python 路线**用于其明确支持的结构化输入、计划检查、知识与私有记录、离线数据比较及模拟恢复。它是可选辅助，不是每项真实任务的统一入口。`task.py`、两个 MethodSpec 模板和模拟连接快照只限定相应本地程序；不得将真实任务伪装成模拟通过，再把输出当成生产批准或平台回执。
...
| 首次使用与分流 | 必要读写模拟能力作为共同入口；按协作方式和需求分流，条件展示 Pipeboard 推荐 | 自动部署连接器、任意自然语言的本地提取、跨宿主真人可用性验证 |
...
| 方法与测试 | 两类 MethodSpec 候选、逐行中英文 SOP 标签、采用回执、声明组件身份约束、单素材测试单元 | 任意 SOP 语义解析、身份真实性与素材理解、绩效排序、等曝光或随机 A/B 执行 |
| 任务入口 | task.py 管理私有档案副本、版本化 MethodSpec、独立计划与模拟恢复 | 独立聊天模型、真实部署、多宿主对话输出保证 |
| 素材选择 | 显式标签、类型、语言、尺寸筛选；已采用方法约束概念/组件声明，以锚定素材和输入顺序选取并解释排除 | 打开或下载素材、视频/OCR 理解、版权核验、模型推荐、质量或效果排序 |
| 计划 | 简报快照、候选元数据 hash、素材筛选、两类测试单元、目标内共享预算、上下文与计划 hash、批次审阅文件 | 真实广告产品字段编译、任意实验设计、平台预检 |
...
| 巡检循环 | 对输入快照诊断，比较基线/目标/当前值，登记本地计划与台账，比较后置快照 | 真实取数、用户审批、实际写入、自动回验上轮待办、后台调度 |
...
Python 首次使用按 `connection_setup → collaboration_intake → business_intake` 推进。`onboarding.py` 先检查本次账户的读取与必要写入快照，再调用 `guidance.py` 产生协作问题或接入推荐；不能用一个 UI 完成标记绕过能力检查。`guided/bring_own` 决定方法引导措辞，平台熟练度与显示偏好独立记录。商业推荐字段只影响呈现，既不参与连接能力判定，也不能替代广告批次授权。这里的能力回执仍是 simulation；宿主实际账户读取不走这份模拟契约。[入口契约与体验](first-run.zh-CN.md)

`operating_loop.py` 的 `ready` 表示所给快照下可供审阅的差异，不是写入授权。`apply` 仅登记计划，`verify` 比较另行提供的后置快照，示例的后置数据是预先保存的。`open` 列出部分台账状态，不自动处理上一轮，也不把 gate 的 `conflict` / `unknown` 纳入该列表。真实任务由宿主另外完成取数、授权核对、工具调用和下一轮回验。[离线循环参考](operating-loop.zh-CN.md)

## Python M2 方法与任务路径

`task.py` 将建任务、更新档案、提议、采用、计划、状态和模拟恢复串在同一个私有任务目录。宿主整理用户回答并调用命令，用户无需编辑 JSON；本地代码不调用 LLM，也没有任意自然语言理解。源 profile 保持不变，任务副本与 MethodSpec 文件分别保存版本。[中英文流程与输入](method-planning.zh-CN.md)

`methodology.py` 只支持 `single_variable` 和 `concept_exploration`。指导路线使用结构化回答，已有方法路线识别明确中英文逐行标签；未识别的非空行保留为 `unsupported`，重复或矛盾条款保留冲突。采用回执绑定完整候选内容，不能用一句“这是老方法”跳过新的结构化确认。

`planning.py` 依据带来源的 `test_identity` 编译测试单元：单变量限定同一概念只变 hook、固定 body/cta/destination；概念探索要求不同 concept 且至少固定 destination。原有元数据筛选继续有效，明确锚定素材和可比数量不足时保留缺口。每单元一个素材，同一目标共享既有总预算，不产生等曝光或随机 A/B 语义。身份只是用户或制作清单声明，未经过视频/画面理解或效果验证。

M1 文本方法仍服务背景与审阅，不自动升级为可执行规则；新 MethodSpec 使用任务目录中的版本化文件，M1 SQLite schema 不变。当前离线方法闭环不代表宿主自然语言提取已通过真人验收，也不具备真实发布适配器。

## 独立 Python 平台适配器的工程设计

如果希望由公开 Python 程序独立提交原生操作，还需建设下列适配器和执行链。这是与宿主工作流并行的工程路线，不能作为宿主使用现成授权工具的前置条件。
...
这些是宿主记录与后续适配器可共用的逻辑职责，不意味着当前代码已经具有七个完整模型。当前 Python 实现使用 JSON 与本地 SQLite；宿主可先用任务记录模板保留证据，无需为了匹配表格而先建立七套服务或数据库。

## 冻结、确认与中断恢复

宿主工作流将业务理解确认与具体批次发布确认分开。先准备一份具体可审阅的计划，再核对其对象、预算与动作是否已经获准；需要用户决定时确认具体批次。范围内步骤连续完成，实质变化生成新差异。创建平台草稿/暂停对象也是外部写入，应包含在相应授权范围。

真实执行需要保存请求身份、返回的原生对象和本次读回。请求结果未知时先查询，不能凭失败文案重建整批；已有配置与基线不同，也不能直接推断由谁修改。回执中的观测值来自带检查时间的读取结果，不能从目标值或硬编码 `ACTIVE` 生成。配置符合、审核通过、可投、已观察到展示和业务表现分开记录；缺证据保留未知。历史状态矛盾及原因见[案例 WB-06](case-studies.zh-CN.md#wb-06--表格写着-active不等于保存了上线证据)。
...
宿主可以将公共知识作为检查参考，将用户方法和当前任务观察保存在私有资料中；读取不授予长期保存权限。用户纠正先作用于当前任务，明确要求保存后才沉淀为可复用约定。下面说明的是 Python 对结构化知识和私有条目的具体实现，不要求每次宿主检索都经过该模块。
