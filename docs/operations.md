...
资料核对日期：2026-09-24。16 张操作卡覆盖素材准备、配置、测试搭建和结果整理。它们既是宿主调用已有工具时的检查参考，也是平台适配器的实现参考。使用前按本次业务、广告产品与工具能力核验；历史任务见[真实案例](case-studies.zh-CN.md)，执行步骤见[宿主工作流](workflow.zh-CN.md)。公开 Python 尚未实现媒体上传、真实预览或平台写入，具体范围见[能力说明](capability-status.zh-CN.md)。
...
历史复核发现过记录表硬编码 ACTIVE、却缺少最终保存读回的情况。回执应分别保存目标值、实际读取值、来源与时间，区分已提交、配置匹配、审核、可投状态以及实际展示/花费；未知不能填成成功。请求超时或证据冲突时，先核对已存在对象，再决定恢复步骤。[案例 WB-06](case-studies.zh-CN.md#wb-06--表格写着-active不等于保存了上线证据) · [任务记录模板](../templates/task-record.zh-CN.md)
