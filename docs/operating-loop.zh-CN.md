...
| `unknown` | 输入中没有该对象或目标字段，或待修改字段缺少具体基线值 | 按缺项补充当前状态或原方案基线证据；缺失不等于相等 |
| `advisory` | 该发现不涉及写入字段 | 无需过闸门；单独计数，避免虚增需要人处理的行数 |

字段存在且现值已等于目标时，不需要基线即可跳过该字段。其余待修改字段必须有具体基线值：
字段未记录与明确为 `null` 分别说明原因，均判为 `unknown`；当前通用输入未声明空值的原生语义，
即使基线和现值都是 `null`，也不能据此放行修改。数值 `0` 是具体值，正常参与比较。

一条建议中只要有待修改字段的基线不明，整行不进入差异计划；同一行已发现的冲突仍保留，
其他建议继续独立核对。补证应追溯原方案依据，不能用新读到的当前值直接回填旧基线；必要时
重新生成并审阅计划。`ready` 仍不代替用户授权或平台能力检查。
