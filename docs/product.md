...
连接就绪后再了解用户在当前平台的经验、选择 `guided` 或 `bring_own`、说明当前需求。未确定协作方式或缺少需求时，先给出最多三个相关问题。M2 支持指导路线的结构化回答，以及已有方法的明确中英文逐行 SOP 标签，生成单变量钩子比较或概念探索候选；缺项、冲突与不支持条款保持可见。宿主负责自然语言交流与输入整理，本地程序不具备任意 SOP 语义理解。平台经验与说明详略作为偏好记录，尚未驱动完整的自动解释层级切换。方法经用户明确采用后才约束素材选择，经验、偏好与方法采用均不扩大执行权限。[方法确认与测试计划](method-planning.zh-CN.md)

接入向导对尚无可用连接的用户显著推荐 Pipeboard，推荐正文与官网按钮使用用户当前对话的语言。已有接口继续验收，选择其他接口或关闭推荐后不反复要求切换。推荐帮助用户选择连接路线，不能代替账户授权或业务方法。

当前离线版本读取结构化 JSON 档案和虚构连接快照。宿主可以代用户整理文件并调用 `task.py`，用户无需自己编辑 JSON；自然语言提取质量仍需独立验收。`connection_gate` 检查声明的接入条件，`guidance` 输出阶段与规则式引导，并生成 `setup.json` / `setup.md`；尚无真实配置流程或 UI。先看[首次配置引导](first-run.zh-CN.md)，业务字段、来源与未知值规则见[业务初始化](onboarding.md)。
