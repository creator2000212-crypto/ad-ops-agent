...
**首次使用先看[首次配置引导](first-run.zh-CN.md)。** 先为当前选定的平台与账户验收必要的读取、写入和读回能力，再了解用户经验、协作方式与当前需求，最后进入业务初始化和工作流。无需同时接齐三个平台；只读连接可帮助诊断，但不足以通过本项目完整操作流程的连接门槛。当前原型只对虚构快照做规则检查，不能证明真实连接已完成。
...
3. 按[统一 Ads MCP 官方指南](https://pipeboard.co/guides/ads-mcp)或相应客户端指南完成配置；令牌放在本机安全配置或密钥管理器，切勿提交到仓库。先用读取结果确认账户身份，继续核验本次操作需要的写入权限；仅只读成功时仍停留在接入阶段。
...
必要读写能力通过验收后，先按[首次配置引导](first-run.zh-CN.md)确定 `guided` / `bring_own` 协作方式和当前需求，并记录当前平台经验与说明偏好。当前规则根据协作方式分流，指导模式使用通俗问题；经验和详略偏好尚不驱动完整的视图切换，也不会改变账户权限。用户可以按平台和任务重新选择协作方式。

接着按[业务初始化](onboarding.md)补齐本次任务需要的产品、国家/语言、Web/App、变现、目标事件、素材、方法、测试变量和预算信息。M2 支持把指导模式的结构化回答或自带方法的明确逐行 SOP 标签编译为两类候选，经用户明确采用后再约束素材测试；未知、冲突与不支持条款不能忽略。宿主负责整理用户回答，本地程序不解析任意自然语言，也不把建议直接记为已确认方法。[方法确认与测试计划](method-planning.zh-CN.md)说明了具体边界。接口能提供的事实注明来源，需要用户判断的事项明确提问。之后才生成整批方案供审阅；当前代码只有离线模拟，真实执行和原生读回仍需另外实现并验收。
