...
接着按[业务初始化](onboarding.md)补齐本次任务需要的产品、国家/语言、Web/App、变现、目标事件、素材、方法、测试变量和预算信息。选择指导模式时收集候选方法所需信息，选择自带方法时请求导入；当前规则不自动生成方法或解析 SOP，也不把任何建议记为已确认方法。接口能提供的事实注明来源，需要用户判断的事项明确提问，缺口保留为未知。之后才生成整批方案，经过预览、确认、执行和原生读回，供投放人员分析结果与决定下一轮方向。
