...
| 你想了解什么 | 阅读 |
|---|---|
| **这套东西是怎么从零搭起来的？架构怎么分层、模块怎么切、数据怎么流、为什么这么选型？** | **[从零搭建：架构、模块、数据流与搭建顺序](build-from-zero.zh-CN.md) · [English](build-from-zero.md)** |
...
| **一个周期性的快循环怎样真正跑起来？判定、闸门、台账、回执分别怎么落地？** | **[投放快循环（可运行）](operating-loop.zh-CN.md) · [English](operating-loop.md)** |
| **不想读文档，想直接看一轮跑完长什么样？** | **[可运行展示 `demo/`](../demo/README.zh-CN.md) —— 里面有已提交的真实回执，不用跑就能看** |
| 优质素材遇到上传故障或疑似误拒，怎样有条件恢复？ | [Meta 素材恢复](meta-creative-recovery.md) |
| 下一步先实现什么，贡献如何验收？ | [路线图](roadmap.md) |

## 建议阅读顺序

- **第一次接触这个项目的人：**先看 [从零搭建](build-from-zero.zh-CN.md)，再看 [demo/](../demo/README.zh-CN.md)。
  前者讲清架构与搭建顺序，后者给出一轮真实运行的回执。两份都不用先跑代码。
...
- **要跑周期性投放的人：**投放快循环 → 实操卡 → 知识库。先看循环的六个步骤与五种闸门结果，再看动作目录里每条动作的「验收」与「止步」。这条路径不依赖任何平台接入，示例即跑即看。
- **只想快速判断这东西值不值得看的人：**直接进 [`demo/`](../demo/README.zh-CN.md)。里面有一轮真实运行的回执与摘要，**不需要跑任何命令**就能看清它能做哪些判断、又拒绝哪些判断。看完再决定要不要读文档。
