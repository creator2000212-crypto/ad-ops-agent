...
| 一轮工作里人和 Agent 各做什么，最后交接什么？ | [demo 五步走读](../demo/README.zh-CN.md) · [离线报告样例](../demo/expected/report.md) |
| 优质素材遇到上传故障或疑似误拒，怎样有条件恢复？ | [Meta 素材恢复](meta-creative-recovery.md) |
| 下一步先实现什么，贡献如何验收？ | [路线图](roadmap.md) |

## 建议阅读顺序

- **第一次接触这个项目的人：**先看 [demo 五步走读](../demo/README.zh-CN.md)，理解一轮工作与输出，再看[搭建与方法演进](build-from-zero.zh-CN.md)。两份都不要求先读代码。
...
- **要复现离线巡检的人：**demo → 投放巡检循环参考。先理解输入与五步输出，再查公式、字段和命令；报告与快照不构成真实执行证据。
