...
- **投放人员与产品协作者：**产品 → 业务初始化 → 知识库 → 按当前问题查实操卡。重点确认理解、批次预览和结果交接能否减少人工工作。
- **接入开发者：**三平台接入全攻略 → 架构 → 知识库 → 连接器契约 → 实操卡 → 路线图。先核对一个平台产品的原生对象、字段与能力，再实现小闭环。
...
这些 `contracts/` 文件是实现参考，当前离线运行时不会自动加载、启用或执行其中的候选能力。已运行的知识库位于 [`knowledge/catalog.json`](../knowledge/catalog.json)，由 `knowledge.py` 加载，与设计契约区分维护。官方来源支持限定范围的事实；工程步骤仍需在所选平台、账户、广告产品和版本下验证。运行时实现在架构和知识库文档中单独列出，避免把设计完成当成功能上线。
